...
- 真實器械 replica + **IR 光學追蹤**(6 相機/每器械 4 標記;**無力回饋裝置**——手感靠視覺 + 眼球機械約束)。
...
3. **撕裂判定**:每元素做**應力(stress)張量特徵值分解**,**最大特徵值 > 門檻 → 標記 "breakable"**
   (門檻依外科醫師回饋調);撕裂方向 = 該特徵值的**主應力方向(特徵向量)**;等向時 = 最大主應力準則。(Dequidt 導論寫 "strain" 是鬆散用詞,正文 §4.3 用 stress。)
...
- Dequidt 是「即時訓練模擬器」等級,追平 = 3D + FEM + 接觸/器械 + 完整術式,屬博士級工程。
