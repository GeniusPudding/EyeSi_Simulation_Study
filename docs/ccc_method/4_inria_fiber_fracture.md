...
**每三角形的纖維方向 θ 一開始怎麼來(初始化)**:不是算出來的,是**建網格時「指定」的**。
囊膜是一個圓盤網格,圓心 = 水晶體軸心。對每個三角形:
```
取三角形中心點 P、圓心 C
徑向 r = normalize(P − C)          # 由中心指向外
纖維方向 f = 垂直於 r 的切線方向     # 即「圓周切線」,f = rotate90(r)
θ = f 相對元素局部座標 x 軸的夾角
```
整片網格掃一遍、每個三角形存一個 θ → 自然形成**同心圓紋路**。
因為用 **Total Lagrangian(相對原始構型)**,θ 只在初始化算一次、之後不變。
> 也就是說「同心圓纖維」= 對每個三角形把纖維設成**「該點到圓心連線的垂直方向(圓周切線)」**。
> 這使圓周方向強(σ̄_F 大)、徑向弱(σ̄_T 小),撕痕沿徑向(往周邊)最省力 → 物理上自然往周邊漂。
