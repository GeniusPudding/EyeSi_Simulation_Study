...
### 2.1 剝離必須是「單一連續裂縫的前緣傳播」(`cap_membrane.py`)

`cap_membrane.py` 的黏附是**逐節點的可斷彈簧**:節點被抬離靜止位置超過
`break_lift = BREAK_FORCE / ADHESION_STIFF = 0.5` 就脫黏。單看這條判據會產生一個
使用者可見的嚴重錯誤 —— **整片瞬間剝落並皺起,而不是從拉的方向長出一片瓣**。

原因是薄膜幾乎不可伸長(`EDGE_STIFFNESS = 2500`):拉起一側,**整片會近乎剛體地一起
抬起**,於是所有節點的 lift 幾乎同時越過 0.5。加上「外緣節點可起裂」這條規則後,
**整圈外緣都是 mesh boundary**,等於整個圓周同時取得各自獨立起裂的資格。

修正是把判據拆成**傳播**與**成核**兩件事(`onAnimateEnd` 的 peel 區塊):

1. **傳播**:節點必須有**至少一個鄰居已脫黏**,才可以脫黏(位於既有裂尖)。
2. **成核**:只有在**還沒有任何節點脫黏**時,才允許成核 **一個** 裂點,取「lift 最大
   的外緣節點」。此後所有脫黏都必須由它傳播 → 全程只有**一條連續裂縫**。
3. **速率上限** `PEEL_RATE`:每步至多這麼多節點脫黏,lift 最大者(裂尖)優先。

第 2 條同時對應臨床目標:CCC 是 *continuous* curvilinear capsulorhexis,**在別處另起
一條裂縫正是它要避免的失敗模式**,模型不該把它抹平。

**實測**(headless、腳本化拉動、`diag_last.csv`;`arcs` = 脫黏扇區形成幾個不相連的弧,
`peelbins` = 36 個 10° 扇區中有幾個含脫黏節點):

| 拉法 | 版本 | 最終殘留黏著 | `peelbins` | `arcs` 峰值 |
|---|---|---|---|---|
| 暴力猛拉 | 只有外緣可起裂(舊) | **0 / 2263(全脫光)** | 36 | **5** |
| 暴力猛拉 | 單一連續裂縫(新) | **1124 / 2263(保住 50%)** | 31 | **1** |
| 溫和拉動 | 舊 | 1603 | 14 | 1 |
| 溫和拉動 | 新 | 1858 | 12 | 1 |

舊版全程 `glued` 每步固定掉 9 個 = `PEEL_RATE` 飽和,代表前緣規則**完全沒有在限制任何
東西**。新版即使維持 `PEEL_RATE=9` 也不再全脫,顯示修掉的是機制本身而非只是拖慢它;
溫和拉動的行為則幾乎不變(略微更難剝,方向正確)。

> [補充] `arcs` / `peelbins` 兩個診斷欄位是為了這次除錯而加進 `diag_last.csv` 的,
> 論文無此對應物;它們是唯一能區分「剝很多」與「到處都在剝」的量測。
